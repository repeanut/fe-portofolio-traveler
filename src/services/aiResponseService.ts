...
class AIResponseService {
  ...
  generateResponse(userMessage: string): AIResponse {
    ...
    const greetingKeywords = [
      'hello', 'hi', 'halo', 'hai', 'hola', 'ciao', 'shalom', 'konnichi', 'nihao', 'annyeonghaseyo', 'privet',
      'selamat', 'salam', 'assalamualaikum', 'shalom', 'namaste', 'sawasdee', 'konnichiwa'
    ];
    
    if (keywords.some(k => greetingKeywords.includes(k))) {
      return this.getGreeting();
    }

    // Enhanced category matching with context awareness
    if (category && responses.categories[category as keyof typeof responses.categories]) {
      ...
      // Smart response selection based on conversation history
      let selectedResponse = categoryResponses[0];
      if (categoryResponses.length > 1) {
        // Use conversation history to select more relevant response
        const historyContext = this.conversationHistory.slice(-3).join(' ').toLowerCase();
        const scoredResponses = categoryResponses.map(response => ({
          ...response,
          score: this.calculateRelevanceScore(response.content, historyContext, keywords)
        }));
        scoredResponses.sort((a, b) => b.score - a.score);
        selectedResponse = scoredResponses[0];
      }
      
      return {
        ...selectedResponse,
        followUpQuestions: this.generateFollowUpQuestions(category, keywords)
      };
    }

    // Enhanced fallback with context awareness
    const fallbacks = responses.fallbacks;
    let selectedFallback = fallbacks[0];
    
    // Select fallback based on conversation context
    if (this.conversationHistory.length > 0) {
      const lastContext = this.conversationHistory[this.conversationHistory.length - 1].toLowerCase();
      if (lastContext.includes('help') || lastContext.includes('assist')) {
        selectedFallback = fallbacks[1] || fallbacks[0];
      } else if (lastContext.includes('understand') || lastContext.includes('clarify')) {
        selectedFallback = fallbacks[2] || fallbacks[0];
      }
    }
    
    return {
      content: selectedFallback,
      suggestions: this.generateContextualSuggestions(keywords)
    };
  }

  private calculateRelevanceScore(content: string, context: string, keywords: string[]): number {
    let score = 0;
    ...
    if (contextLower.includes('budget') && contentLower.includes('price')) score += 3;
    if (contextLower.includes('time') && contentLower.includes('when')) score += 3;
    if (contextLower.includes('how') && contentLower.includes('guide')) score += 3;
    
    return score;
  }

  private generateFollowUpQuestions(category: string, _keywords: string[]): string[] {
    const followUpTemplates = {
      destinations: [
        "Would you like more details about this destination?",
        "What specific information interests you?",
      ...
      transportation: [
        ...
        "Do you need help with routes?",
        "Are you looking for specific transport options?"
      ],
      food: [
      ...
      culture: [
        "Want to know about local customs?",
        "Interested in cultural events?",
        "Need etiquette tips?"
      ],
      practical: [
        "Need help with planning?",
        "What's your travel timeframe?",
    ...
    return followUpTemplates[category as keyof typeof followUpTemplates] || [
  ...
  private generateContextualSuggestions(keywords: string[]): string[] {
    const suggestions = [];
    
    // Generate suggestions based on detected keywords
    if (keywords.some(k => ['bali', 'jakarta', 'yogyakarta'].includes(k))) {
      suggestions.push("Tell me more about destinations", "Popular attractions", "Best time to visit");
    }
    
    if (keywords.some(k => ['hotel', 'resort', 'stay'].includes(k))) {
      suggestions.push("Budget accommodation", "Luxury options", "Booking tips");
    }
    
    if (keywords.some(k => ['flight', 'train', 'bus', 'transport'].includes(k))) {
      suggestions.push("Transportation guide", "Domestic flights", "Local transport");
    }
    
    if (keywords.some(k => ['food', 'eat', 'restaurant', 'makanan'].includes(k))) {
      suggestions.push("Local cuisine", "Street food guide", "Dietary options");
    }
    
    if (keywords.some(k => ['culture', 'tradition', 'custom'].includes(k))) {
      suggestions.push("Cultural etiquette", "Local customs", "Religious sites");
    }
    
    if (keywords.some(k => ['visa', 'budget', 'cost', 'price'].includes(k))) {
      suggestions.push("Travel budget", "Visa requirements", "Money-saving tips");
    }
    
    // Default suggestions if no specific keywords detected
    if (suggestions.length === 0) {
      suggestions.push("Popular destinations", "Travel tips", "Contact support", "Best time to visit", "Visa requirements", "Plan your trip", "Discover Indonesia");
    }
