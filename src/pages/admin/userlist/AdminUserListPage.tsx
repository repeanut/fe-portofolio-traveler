...
const AdminUserListPage: React.FC = () => {
    ...
    try {
      ...
      if (result.success) {
        const formattedUsers = result.data.users.map((user: any) => ({
          ...
          displayName: user.displayName || user.username || 'N/A',
          role: user.role || 'user',
          provider: user.provider || 'local',
          isEmailVerified: user.provider === 'google' ? 'Yes' : (user.isEmailVerified ? 'Yes' : 'No'),
          lastLogin: user.lastLogin ? new Date(user.lastLogin).toLocaleString() : 'Never',
          createdAt: user.created_at ? new Date(user.created_at).toLocaleString() : 'N/A',
          userType: user.userType || 'regular',
          source: user.source || 'Main System'
        }));
  ...
  const handleItemsPerPageChange = (newItemsPerPage: number) => {
    ...
    setTotalPages(Math.ceil(userData.length / newItemsPerPage));
  };
