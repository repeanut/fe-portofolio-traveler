    ...
    const fetchTravelJournals = async () => {
        ...
        try {
            ...
            if (result.success) {
                ...
                const transformedHighlights = (result.data.journals || []).map((journal: any) => ({
                    id: journal.id,
                    label: journal.name,
                    coverImage: journal.cover,
                    timestamp: journal.timestamp || 'Just now',
                    stories: (journal.images || []).map((img: string, index: number) => ({
            ...
            <div className="max-w-7xl mx-auto px-4">
                ...
                {isLoading && (
                    <div className="flex justify-center items-center py-12">
                        <div className="text-center">
                            ...
                            <span className="text-slate-600">Loading travel journals...</span>
                        </div>
                    </div>
                )}
                ...
                {!isLoading && !error && highlights.length === 0 && (
                    <div className="text-center py-12">
                        <p className="text-slate-500">No travel journals available yet.</p>
                    </div>
                )}

                {/* Carousel Container - Only show when not loading and has data */}
                {!isLoading && !error && highlights.length > 0 && (
